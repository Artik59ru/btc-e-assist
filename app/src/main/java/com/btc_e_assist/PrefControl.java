...
public class PrefControl {
    ...
    public boolean getCurrentTradeSwitchState() {
        return preferences.getBoolean(TRADE_SWITCH_STATE, false);
    }

    public synchronized void setTradeSwitchState(boolean state) {
        Editor editPref = preferences.edit();
        editPref.putBoolean(TRADE_SWITCH_STATE, state);
        editPref.commit();
    }
}
